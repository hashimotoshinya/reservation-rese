...
---
#### 追記
追加実装の『AWS	ストレージをS3、バックエンドをEC2、データベースをRDSとして環境を構築する』『環境の切り分け	開発環境と本番環境の切り分けを行う』についてクライアント（コーチ）との要件確認、また自身でも調べてみましたが、理解が曖昧な感じでの実装となった感じがします。その結果AWSにはローカルと同様に動作するものを、環境の切り分けについてREADMEに記載と言う形での提出にしました。こうではない！であるなら少し評価シートで可能であれば解説いただければと思います。採点とともによろしくお願いいたします。
